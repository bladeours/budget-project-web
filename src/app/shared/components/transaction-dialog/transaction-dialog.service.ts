  ...
  public getTransactionInput(formGroup: FormGroup): TransactionInput {
    ...
    switch (formGroup.get("type")?.value as unknown as TransactionType) {
      case TransactionType.Expense:
        if (formGroup.get("subCategoryExpense")?.value !== undefined && formGroup.get("subCategoryExpense")?.value !== null) {
          subCategoryHash = (formGroup.get("subCategoryExpense")?.value as unknown as Category).hash;
        }
        categoryHash = (formGroup.get("right")?.value as unknown as Category).hash;
        accountFromHash = (formGroup.get("left")?.value as unknown as Account).hash;
        break;
      case TransactionType.Income:
        if (formGroup.get("subCategoryIncome")?.value !== undefined && formGroup.get("subCategoryIncome")?.value !== null ) {
          subCategoryHash = (formGroup.get("subCategoryIncome")?.value as unknown as Category).hash;
        }
        categoryHash = (formGroup.get("left")?.value as unknown as Category).hash;
        accountToHash = (formGroup.get("right")?.value as unknown as Account).hash;
        break;
