...
import { TransactionDialogComponent } from 'src/app/features/transactions/dialog/transaction-dialog/transaction-dialog.component';
import {
  Account,
  AccountType,
  Category,
...
export class NavbarComponent {
  ...
  setAccounts() {
    this.graphqlService
      .getAccounts({
        ...
        accountTypeFilters: [
          { field: 'accountType', value: AccountType.Savings },
        ],
      })
      .subscribe({
        next: (v) => {
          this.accountsSavings = (v.data.getAccounts as Account[]).filter(
            (a) => !a.archived,
          );
          this.accountsSavingsArchived = (
    ...
    this.graphqlService
      .getAccounts({
        logicOperator: LogicOperator.And,
        accountTypeFilters: [
          { field: 'accountType', value: AccountType.Regular },
        ],
      })
      .subscribe({
        next: (v) => {
          this.accountsRegular = (v.data.getAccounts as Account[]).filter(
            (a) => !a.archived,
          );
  ...
  setCategories() {
    this.graphqlService.getCategoriesIncomeHashName().subscribe({
      next: (v) => {
        this.categoryIncome = (v.data.getCategories as Category[]).filter(
          (c) => !c.archived,
        );
        this.categoryArchivedIncome = (
          v.data.getCategories as Category[]
        ).filter((c) => c.archived);
      },
    });
    this.graphqlService.getCategoriesExpenseHashName().subscribe({
      next: (value) => {
        this.categoryExpense = (value.data.getCategories as Category[]).filter(
          (c) => !c.archived,
        );
        this.categoryArchivedExpense = (
          value.data.getCategories as Category[]
        ).filter((c) => c.archived);
      },
    });
  }
